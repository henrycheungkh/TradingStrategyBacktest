...
class CorrelationOnSpecificTimeSectionStrategy:
    ...
    def __init__(self, StrategyLabel, AnalysisContext, StartTimeInStdUnit, EndTimeInStdUnit, TimeIntervalInStdUnit, OBS_PERIOD_MOVEMENT_THRESHOLD = [0, 0.001], ObsPeriodMovementRange = 0, ObsPeriodExponentialStep = 0.0, StopLossPerTrade = 0, TakeProfitPerTrade = 0, ObsDateIDOffset = 0, TradeEntryDateIDOffset = 0, TradeIDSortingSegmentKeyColumnsCount = 2, PreFilterDataByTime = False, GPUMode = "CUDA", InitialResultCacheSize = None, TotalBatchIndex=0, BatchListDatabaseName = 'finance_fdata_master', BatchListTableName = 'fdata_backtest_batch'):
        ...
        self.ObsPeriodMovementRange = ObsPeriodMovementRange
        self.ObsPeriodExponentialStep = ObsPeriodExponentialStep
        ...
        if GPUMode == "CUDA":
            self.trade_record = CUDACorrelationOnSpecificTimeSectionStrategy(self.AnalysisContext.IntradayPricesData.DataMatrix['TRADES_close_adj'], self.AnalysisContext.IntradayPricesData.DataMatrix['date id'], self.AnalysisContext.IntradayPricesData.DataMatrix['TimeInStandardUnit'], self.StartTimeInStdUnit, self.EndTimeInStdUnit, self.TimeIntervalInStdUnit, len(DateList), ObsPeriodMovementThreshold = self.OBS_PERIOD_MOVEMENT_THRESHOLD, ObsPeriodMovementRange = self.ObsPeriodMovementRange, ObsPeriodExponentialStep = self.ObsPeriodExponentialStep, StopLossPerTrade = self.StopLossPerTrade, TakeProfitPerTrade = self.TakeProfitPerTrade, ObsDateIDOffset = self.ObsDateIDOffset, TradeEntryDateIDOffset = self.TradeEntryDateIDOffset, InitialResultCacheSize = self.InitialResultCacheSize, TimeFrame = self.TimeFrame)
    ...
    def PrepareEmailAlert(StrategyExecutionID, MessageFirstLine, ticker, ObservationTimeText, ObservationMovement, TakeProfitBps, StopLossBps, TradeEntryTime, TickSize = 0.1, AppendixFirstLine = '', TradeForceExitTime = '', BackTestSharpeRatio = 0, BackTestTradeSampleSize = 0, BackTestPeriod = '', isMeanReversing=True, UpdateStrategyExecutionConfig = False):
        today = date.today()
        ...
        Message = str(StrategyExecutionID) + '.<BR>' + MessageFirstLine + '<BR><table border =1>'
        ...
        Appendix = str(StrategyExecutionID) + '.<BR>' + AppendixFirstLine + '<BR><table border=1><tr><td>Backtest Sharpe Ratio</td><td>' + str(BackTestSharpeRatio) + '</td></tr>'
        ...
        if UpdateStrategyExecutionConfig:
            PriorDateMinusMovement = float(PriorDateMinusMovement)
            PriorDatePlusMovement = float(PriorDatePlusMovement)
            StrategyExecutionID = int(StrategyExecutionID)
            print(str(PriorDateMinusMovement) + ', ' + str(PriorDatePlusMovement) + ', ' + str(StrategyExecutionID))

            statement = text("""UPDATE strategy_execution_config SET ParameterValue1 = :ParameterValue1, ParameterValue2 = :ParameterValue2 WHERE StrategyExecutionID = :StrategyExecutionID""")
            line = { "ParameterValue1": PriorDateMinusMovement, "ParameterValue2": PriorDatePlusMovement, "StrategyExecutionID": StrategyExecutionID}
            engine = DBUtil.GetSQLAlchemyEngine()
            # engine.execute(statement, **line)
            with engine.connect() as conn:
                result = conn.execute(statement, line)
            print('strategy_execution_config updated')
...
class BacktestCorrelationOnSpecificTimeSectionStrategy(FuturesStrategyBacktest):
                    ...
                    for TakeProfitPerTrade in self.StopLossTakeProfitList:
                        for ObsDateIDOffset in self.ObsDateIDOffsetList:
                            for TradeEntryDateIDOffset in self.TradeEntryDateIDOffsetList:
                                ...
                                backtest_result = CorrelationOnSpecificTimeSectionStrategy(self.StrategyLabel, self.AnalysisContext, self.StartTimeInStdUnit, self.EndTimeInStdUnit, self.TimeIntervalInStdUnit, OBS_PERIOD_MOVEMENT_THRESHOLD = threshold_list, ObsPeriodMovementRange = ObsPeriodMovementRange, ObsPeriodExponentialStep = self.ObsPeriodExponentialStep, StopLossPerTrade = StopLossPerTrade, TakeProfitPerTrade = TakeProfitPerTrade, ObsDateIDOffset = ObsDateIDOffset, TradeEntryDateIDOffset = TradeEntryDateIDOffset, TradeIDSortingSegmentKeyColumnsCount = self.TradeIDSortingSegmentKeyColumnsCount, GPUMode = self.GPUMode, InitialResultCacheSize = self.InitialResultCacheSize, TotalBatchIndex=TotalBatchIndex, BatchListDatabaseName = self.BatchListDatabaseName, BatchListTableName = self.BatchListTableName )
    ...
    def loadStrategyParameters(self):
        ...
        self.ObsPeriodExponentialStep = self.BacktestParameterDF.loc[0, 'ParameterValue5']
