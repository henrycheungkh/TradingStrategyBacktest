...
class FuturesPriceReader(SpotPriceReader):
    DefaultMarketTimeSection = {'1 min':[8*60+30, 9*60+30, 11*60, 15*60, 16*60], '10 secs':[(8*60+30)*6, (9*60+30)*6, (11*60)*6, (15*60)*6, (16*60)*6], '5 mins':[8*12+6, 9*12+6, 11*12, 15*12, 16*12]}
    ...
    def AttachMarketTimeSectionID(self, df, TimeInStandardUnitColumnName = 'TimeInStandardUnit'):
        ...
        df.loc[df[TimeInStandardUnitColumnName] > self.MarketTimeSection[len(self.MarketTimeSection) - 1], 'MarketTimeSectionID'] = 0
        return df
    ...
    def LoadRawData(self):
        ...
        if (self.TimeFrame == "5 mins"):
            Prices_df['TimeInStandardUnit'] = Prices_df['Hour'] * 12 + Prices_df['Minute'] / 5
        if (self.TimeFrame == "1 min"):
        ...
        if (self.TimeFrame == "5 mins"):
            self.TimeIDMapping['TimeInStandardUnit'] = self.TimeIDMapping['Hour'] * 12 + self.TimeIDMapping['Minute'] / 5
        ...
        if self.GPUMode:
            ...
            if self.DebugFilepath is not None:
                ...
                try:
                    ...
                    close_price_matrix = close_price_matrix[:,:2]
                    df = pd.DataFrame(data=close_price_matrix, columns=['close_adj 1', 'close_adj 2'])
                    df.to_csv(self.DebugFilepath + r'\TRADES_close_adj_AfterFillDataMatrix_FirstTwoTicker.csv', index=False)
        ...
        if self.KeepDataframeData:
            self.Prices = Prices_df
        # else:
        #     self.ActiveContractPrices = None

        
    def ContangoAdjustment(ActiveContractPrices, RolloverDate_df):
        
        print('RolloverDate_df is')
        print(RolloverDate_df)
        # RolloverDate_df.to_csv(r'g:\temp\RolloverDate_df.csv')
        ...
        if len(RolloverDate_df) > 0:
            ...
            while PendingContangoAdjCount > 0:
                ...
                ActiveContractPrices.loc[ActiveContractPrices['Contango'].notnull(), 'high_adj'] = ActiveContractPrices['high_adj'] + ActiveContractPrices['Contango']
                ActiveContractPrices.loc[ActiveContractPrices['Contango'].notnull(), 'low_adj'] = ActiveContractPrices['low_adj'] + ActiveContractPrices['Contango']
                ActiveContractPrices.loc[ActiveContractPrices['Contango'].notnull(), 'open_adj'] = ActiveContractPrices['open_adj'] + ActiveContractPrices['Contango']
                ActiveContractPrices.loc[ActiveContractPrices['Contango'].notnull(), 'close_adj'] = ActiveContractPrices['close_adj'] + ActiveContractPrices['Contango']
...
class IBFuturesPriceReader(FuturesPriceReader):
    def GetDataframeFromDB(self, TickerFilter = []):
        DATABASE_FUT_HIST_10SECS_SUFFIX = {2021:"_2021", 2022:"_2022", 2023:"_2023", 2024:"_2024", 2025:""}
...
class FXFuturesPriceReader(FuturesPriceReader):
    def GetDataframeFromDB(self, TickerFilter = []):
        ...
        return pd.read_sql(sql,con=DBUtil.GetSQLAlchemyEngine(DatabaseName=Config.CONFIG_MYSQL_CONNECTION_DATABASE)) 
...
class DailySpotPriceReader(SpotPriceReader):
    ...
    def LoadRawData(self):
        ...
        Prices = pd.read_sql(sql,con=DBUtil.GetSQLAlchemyEngine(DatabaseName=Config.CONFIG_MYSQL_CONNECTION_DATABASE)) 
...
def GetResampledData(df_full_data, resample_frequency = 'D'):
    ...
    for ticker in ticker_list:
        ...
        df2 = df[['tDateTime', 'high_adj', 'low_adj', 'open_adj', 'close_adj']].set_index('tDateTime')
        
        df2 = df2.resample('D').agg({'open_adj': 'first', 
                                          'high_adj': 'max', 
                                          'low_adj': 'min', 
                                          'close_adj': 'last'}).reset_index()
        
        df2 = df2[df2.notnull().all(axis=1)]
        
        df = df1.merge(df2, on='tDateTime', how='inner')
