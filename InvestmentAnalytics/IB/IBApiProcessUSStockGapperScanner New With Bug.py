...
class IBapiUSStocksGapperScanner(IBapiDataReader):
    ...
    def RunProcess(self, IBProcessHub):
        print('IBapiUSStocksGapperScanner.RunProcess')
        print('self.TickerList is ')
        print(self.TickerList)
        self.TickerList.to_csv(r'd:\temp\TickerList.csv')
        ...
        while ((US_time.hour < ScannerEndTime.hour) or ((US_time.hour == ScannerEndTime.hour) and (US_time.minute < ScannerEndTime.minute))):
            ...
            if self.StaleScanDontCountIncrement > 5:
                print('StaleScanDontCountIncrement > 5, should I restart now?')
                time.sleep(120)
                
            if self.ContinuousLargeGapperCount > 20:
                print('ContinuousLargeGapperCount > 20')
                # Now_time = datetime.now() 
        ...
        if not ((US_time.hour < ScannerEndTime.hour) or ((US_time.hour == ScannerEndTime.hour) and (US_time.minute < ScannerEndTime.minute))):
            print('Scanner time out')
            
        self.IBProcessHub.Alive = False
        return None
    def get30MAVolCount(self, ticker):
        try:
            ...
            df = pd.read_sql(sql, con=DBUtil.GetSQLAlchemyEngine())
    ...
    def TriggerUpdateHistoricalPricesOfTicker(self, batch_id):
        ...
        # print('In TriggerUpdateHistoricalPricesOfTicker, batch_id is ' + str(batch_id) + ' and TickerIndex is ' + str(TickerIndex) + ' and self.BaseReqIDCount is ' + str(self.BaseReqIDCount) + ' and self.TickerListLength is ' + str(self.TickerListLength) + ' and self.TickerList.loc[TickerIndex, symbol] is ' + str(self.TickerList.loc[TickerIndex, 'symbol']))
    ...
    def getGapperList(self, ticker_filter=""):
        try:
            ...
            return pd.read_sql("SELECT * FROM `fdata_us_gapper_list` WHERE Today_Vol * CurrentPrice > " + str(IBapiUSStocksGapperScanner.GapperDollarVolumeThreshold) + " AND CaptureDate = '" + datetime.now(timezone('America/New_York')).strftime('%Y-%m-%d') + "' "  + ticker_filter + " ORDER BY ABS((CurrentPrice - PriorDayClose)/PriorDayClose) DESC", con=DBUtil.GetSQLAlchemyEngine())
        except:
            return None
    def getLastScanIndex(self, ticker_filter=""):
        try:
            ...
            df = pd.read_sql("SELECT * FROM `fdata_us_gapper_list_last_scan_index` WHERE CaptureDate = '" + datetime.now(timezone('America/New_York')).strftime('%Y-%m-%d') + "'", con=DBUtil.GetSQLAlchemyEngine())
    ...
    def historicalData(self, reqId:int, bar: BarData):
        ...
        if self.ReqMode[batch_id] == 1:
            ...
            TickerIdx = self.TickerIndexOffset[batch_id] * self.BaseReqIDCount + batch_id
            d['ticker'] = self.TickerList.loc[TickerIdx, 'ticker']
            self.data[batch_id].append(d)
            # print('in historicalData, for self.ReqMode[batch_id] = 1, d =')
            # print(d)
            # print('in historicalData, for self.ReqMode[batch_id] = 1, self.TickerDict =')
            # print(self.TickerDict)
            # print('in historicalData, for self.ReqMode[batch_id] = 1, self.TickerList =')
            # print(self.TickerList)
        ...
        # print("HistoricalData. ReqId:", reqId, "BarData.", bar ,"Ticker.", self.TickerList.loc[TickerIndex, 'ticker'], "Close.", d['close'])
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        if self.ContinuousLargeGapperCount > 10:
            ...
            if ((self.ScanDoneCount < 100) and (self.ScanDoneCount % 10 == 0)) or self.ScanDoneCount % 50 == 0:
            ...
            if batch_id < self.BaseReqIDCount:
                ...
                sub_batch_id = batch_id + IBapiUSStocksGapperScanner.ShortListReqIDCount - self.AllReqIDCount
                TickerIndex = self.GapperTickerIndexOffset[sub_batch_id] * IBapiUSStocksGapperScanner.ShortListReqIDCount + sub_batch_id
                print('in historicalDataEnd, Gapper rescan, batch_id = ' + str(batch_id) + ', sub_batch_id = ' + str(sub_batch_id) + ', TickerIndex = ' + str(TickerIndex) + ', self.GapperTicker[sub_batch_id] = ' + str(self.GapperTicker[sub_batch_id]))
    ...
    def error(self, reqId: TickerId, errorCode: int, errorString: str):
        ...
        if not errorString == 'Not connected' :
            ...
            if not 'HMDS query returned no data' in errorString and not 'fractional share size' in errorString and not 'Historical Market Data Service' in errorString:
    ...
    def UpdateGapper(self, sub_batch_id, bid_ask_spread = 0):
        ...
        if bid_ask_spread == 0:
            statement = text("""UPDATE fdata_us_gapper_list SET CurrentPrice = :CurrentPrice, Today_Vol = :Today_Vol, 30MA_Vol = :30MA_Vol WHERE CaptureDate = :CaptureDate AND ticker = :ticker""")
            line = { "CurrentPrice": float(self.ReqIDPrice[batch_id]), "Today_Vol": float(self.ReqIDVol[batch_id]), "30MA_Vol": float(MA30Vol), "CaptureDate": US_time, "ticker": self.GapperTicker[sub_batch_id]}
        else:
            statement = text("""UPDATE fdata_us_gapper_list SET CurrentPrice = :CurrentPrice, Today_Vol = :Today_Vol, 30MA_Vol = :30MA_Vol, BidAskSpread = :BidAskSpread WHERE CaptureDate = :CaptureDate AND ticker = :ticker""")
            line = { "CurrentPrice": float(self.ReqIDPrice[batch_id]), "Today_Vol": float(self.ReqIDVol[batch_id]), "30MA_Vol": float(MA30Vol), "BidAskSpread": float(bid_ask_spread / self.ReqIDPrice[batch_id]), "CaptureDate": US_time, "ticker": self.GapperTicker[sub_batch_id]}

        DBUtil.GetSQLAlchemyEngine().execute(statement, **line)
    ...
    def AddToGapperList(self, TickerIndex, batch_id):
        ...
        statement = text("""INSERT INTO fdata_us_gapper_list (CaptureDate, ticker, Sector, Industry, PriorDayClose, CurrentPrice, 30MA_Vol, Today_Vol, MarketCap, FreeFloat, BidAskSpread, FirstCaptureDatetime, FirstCapturePrice) VALUES (:CaptureDate, :ticker, :Sector, :Industry, :PriorDayClose, :CurrentPrice, :30MA_Vol, :Today_Vol, :MarketCap, :FreeFloat, :BidAskSpread, :FirstCaptureDatetime, :FirstCapturePrice) ON DUPLICATE KEY UPDATE CurrentPrice = :CurrentPrice, 30MA_Vol = :30MA_Vol, Today_Vol = :Today_Vol""")
        # line = { "CaptureDate": US_time, "ticker": self.TickerList.loc[TickerIndex, 'ticker'], "Sector": self.TickerList.loc[TickerIndex, 'Sector'], "Industry": self.TickerList.loc[TickerIndex, 'Industry'], "PriorDayClose": self.TickerList.loc[TickerIndex, 'PriorDayClose'], "CurrentPrice": self.TickerList.loc[TickerIndex, 'CurrentPrice'], "30MA_Vol": MA30Vol, "Today_Vol": self.ReqIDVol[batch_id], "MarketCap": MarketCap, "FreeFloat": FreeFloat, "BidAskSpread": 0.1, "FirstCaptureDatetime": US_time, "FirstCapturePrice": self.TickerList.loc[TickerIndex, 'CurrentPrice'] }
        line = { "CaptureDate": US_time, "ticker": self.TickerList.loc[TickerIndex, 'ticker'], "Sector": self.TickerList.loc[TickerIndex, 'Sector'], "Industry": self.TickerList.loc[TickerIndex, 'Industry'], "PriorDayClose": float(self.TickerList.loc[TickerIndex, 'PriorDayClose']), "CurrentPrice": float(self.TickerList.loc[TickerIndex, 'CurrentPrice']), "30MA_Vol": float(MA30Vol), "Today_Vol": float(self.ReqIDVol[batch_id]), "MarketCap": float(MarketCap), "FreeFloat": float(FreeFloat), "BidAskSpread": 0.1, "FirstCaptureDatetime": US_time, "FirstCapturePrice": float(self.TickerList.loc[TickerIndex, 'CurrentPrice']) }
        DBUtil.GetSQLAlchemyEngine().execute(statement, **line)
    ...
    def UpdateHistoricalPricesOfTicker(self, batch_id):
        ...
        for index, row in df.iterrows():
            ...
            try:
                print('row is')
                print(row)
                print('row[\'DateTime\'].astype(str) is')
                print(row['DateTime'].astype(str))
                statement = text("""INSERT INTO finance_fdata_price_30min_ib.fdata_price_30min_ib (ticker, DataType, timeframe, DateTime, high, low, open, close, vol) VALUES (:ticker, :DataType, :timeframe, :DateTime, :high, :low, :open, :close, :vol) ON DUPLICATE KEY UPDATE high = :high, low = :low, open = :open, close = :close, vol = :vol""")
                # line = { "ticker": row['ticker'], "DataType": 'TRADES', "timeframe": '30 mins', "DateTime": row['DateTime'], "high": float(row['high']), "low": float(row['low']), "open": float(row['open']), "close": float(row['close']), "vol": float(row['volume'])}
                line = { "ticker": row['ticker'], "DataType": 'TRADES', "timeframe": '30 mins', "DateTime": row['DateTime'].astype(str), "high": float(row['high']), "low": float(row['low']), "open": float(row['open']), "close": float(row['close']), "vol": float(row['volume'])}
                DBUtil.GetSQLAlchemyEngine().execute(statement, **line)
                print('UpdateHistoricalPricesOfTicker success')
            except Exception as inst:
                print('Error in UpdateHistoricalPricesOfTicker')
                print(inst)  
...
class IBapiUSStocksDayEndHistoricalDataReader(IBapiDataReader):
    ...
    def RunProcess(self, IBProcessHub):
        print('IBapiUSStocksDayEndHistoricalDataReader.RunProcess')
        print('self.TickerList is ')
        print(self.TickerList)
        ...
        print('len of TickersBatch is ' + str(len(TickersBatch)))
