...
#     gpu_core_block_count = math.ceil(first_dimension_size/GPU_CORE_BLOCK_SIZE)
    ...
    mod = SourceModule("""
      ...
      {
        if (ObsPeriodExponentialStep == 0.0) {
            return correlation_on_specific_time_section_analysis_linear(AdditionalResultCount, ticker_size, gpu_core_block_count, time_size, StartTimeInStdUnit, EndTimeInStdUnit, TimeIntervalInStdUnit, obs_movement_threshold_count, obs_movement_range, stop_loss, take_profit, obs_date_id_offset, trade_entry_date_id_offset, ErrorCode, InitialResultCacheSize, TRADE_RESULT_COLUMN_COUNT, trade_result_count, close_data, date_id_data, time_std_unit_data, obs_movement_threshold, trade_result);
        } 
        return 0;
      }
