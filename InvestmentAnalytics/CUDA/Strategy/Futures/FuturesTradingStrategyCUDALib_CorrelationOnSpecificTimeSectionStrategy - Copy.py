...
//            thread_Index = threadIdx.x * GPU_CORE_BLOCK_SIZE_Y * GPU_CORE_BLOCK_SIZE_Z + threadIdx.y * GPU_CORE_BLOCK_SIZE_Z + threadIdx.z;
//            first_time_id = k * GPU_CORE_BLOCK_SIZE_X * GPU_CORE_BLOCK_SIZE_Y * GPU_CORE_BLOCK_SIZE_Z + thread_Index;
            first_time_id = k * blockDim.x * gridDim.x + thread_Index;
            first_time_id_offset = first_time_id * second_dimension_size;
            if (first_time_id < first_dimension_size && first_time_id > 0) {
              ...
              for (int obs_movement_threshold_index = 0; obs_movement_threshold_index < obs_movement_threshold_count; obs_movement_threshold_index++ ) {
                ...
                for (int ticker_id = 0; ticker_id < second_dimension_size; ticker_id++) {
                  ...
                  if (first_time_in_std_unit >= StartTimeInStdUnit && first_time_in_std_unit <= EndTimeInStdUnit && (first_time_in_std_unit - StartTimeInStdUnit) % TimeIntervalInStdUnit == 0) {
                    ...
                    for (int second_time_id = first_time_id + 1; second_time_id < first_dimension_size; second_time_id++) {
                      second_time_id_offset = second_time_id * second_dimension_size;
                      ...
                      if (second_date_id == first_date_id + obs_date_id_offset && (second_time_in_std_unit > first_time_in_std_unit || obs_date_id_offset > 0) && second_time_in_std_unit <= EndTimeInStdUnit && (second_time_in_std_unit - StartTimeInStdUnit) % TimeIntervalInStdUnit == 0) {
                        ...
                        for (int third_time_id = second_time_id + 1; third_time_id < first_dimension_size; third_time_id++) {
                          third_time_id_offset = third_time_id * second_dimension_size;
                              ...
                              if (position_flag != 0) {
                                ...
                                if((stop_loss != 0) || (take_profit != 0)) {
                                  for (int path_scan_time_id = second_time_id + 1; path_scan_time_id < third_time_id; path_scan_time_id++) {
                                    if (((position_flag > 0) && (((stop_loss != 0) && (close_data[path_scan_time_id * second_dimension_size + ticker_id] <= (close_data[second_time_id_offset + ticker_id] * (1 - stop_loss)))) || ((take_profit != 0) && (close_data[path_scan_time_id * second_dimension_size + ticker_id] >= (close_data[second_time_id_offset + ticker_id] * (1 + take_profit)))))) || ((position_flag < 0) && (((stop_loss != 0) && (close_data[path_scan_time_id * second_dimension_size + ticker_id] >= (close_data[second_time_id_offset + ticker_id] * (1 + stop_loss)))) || ((take_profit != 0) && (close_data[path_scan_time_id * second_dimension_size + ticker_id] <= (close_data[second_time_id_offset + ticker_id] * (1 - take_profit))))))) {
                                      stop_time_id = path_scan_time_id;
                                      third_time_close_price = close_data[path_scan_time_id * second_dimension_size + ticker_id];
                                      break;
                                    }
                                  }
                                } 
