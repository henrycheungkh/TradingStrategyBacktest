...
class IBapiUSStocksGapperScanner(IBapiDataReader):
    ...
    def getContract(self, symbol, secType, exchange, currency, primaryExchange = 'NONE'):
        contract = Contract()
    ...
    def historicalData(self, reqId:int, bar: BarData):
        batch_id = reqId - self.RequestID_Range[0]
        d = vars(bar)
        if self.PreRun:
    ...
    def RunProcess(self, IBProcessHub):
        ...
        contract = self.getContract("AAPL", "STK", "SMART", "USD", "NONE")
        self.IBProcessHub.reqHistoricalData(self.RequestID_Range[0], contract, self.DataEndTime, "5 D", "1 day", 'TRADES', 0, 1, False, [])
