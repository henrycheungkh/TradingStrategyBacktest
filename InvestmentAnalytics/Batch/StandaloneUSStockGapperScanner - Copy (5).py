    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        if self.PreRun:
            ...
            if ( self.TickerList.loc[self.TickerIndex[thread_id], 'CurrentPrice'] * self.ReqIDVol[thread_id] > IBapiUSStocksGapperScanner.GapperDollarVolumeThreshold) and ((PriceGap > IBapiUSStocksGapperScanner.GapThreshold) or (PriceGap < - IBapiUSStocksGapperScanner.GapThreshold)):
                print('*** historicalDataEnd' + ', symbol = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'symbol']) + ', PriceGap = ' + str(PriceGap) + ', CurrentPrice = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'CurrentPrice']) + ', PriorDayClose = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'PriorDayClose'])  + ', TradingVolume = ' + str(self.ReqIDVol[thread_id]) + ', DollarVolumeThreshold = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'CurrentPrice'] * self.ReqIDVol[thread_id])+ ', cycle = ' + str(self.cycle) + ',  thread_id = ' + str(thread_id) )
                self.AddToGapperList(self.TickerIndex[thread_id], thread_id)
            else:
                # print('* historicalDataEnd, cycle = ' + str(self.cycle) + ',  thread_id = ' + str(thread_id) + ', symbol = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'symbol']) + ', CurrentPrice = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'CurrentPrice']) + ', PriorDayClose = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'PriorDayClose']) + ', PriceGap = ' + str(PriceGap) + ', TradingVolume = ' + str(self.ReqIDVol[thread_id]) + ', DollarVolumeThreshold = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'CurrentPrice'] * self.ReqIDVol[thread_id]))
                pass
            self.ReqIDIdle[thread_id] = True
