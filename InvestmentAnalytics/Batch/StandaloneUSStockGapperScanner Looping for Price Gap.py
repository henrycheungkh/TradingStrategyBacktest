...
class IBapiUSStocksGapperScanner(IBapiDataReader):
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        if self.PreRun:
            ...
            print('historicalDataEnd, thread_id = ' + str(thread_id) + ', symbol = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'symbol']) + ', CurrentPrice = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'CurrentPrice']) + ', PriorDayClose = ' + str(self.TickerList.loc[self.TickerIndex[thread_id], 'PriorDayClose']) + ', PriceGap = ' + str(PriceGap))
    ...
    def RunProcess(self, IBProcessHub):
        ...
        # while (True):
            print('while loop, thread_id = ' + str(thread_id) + ', self.ReqIDIdle[' + str(thread_id) + '] = ' + str(self.ReqIDIdle[thread_id]) + ', self.ReqIDIdle[0] = ' + str(self.ReqIDIdle[0]))
            time.sleep(5)
            if self.ReqIDIdle[thread_id]:
                self.TriggerScanning(thread_id)
            # print('while loop after TriggerScanning, thread_id = ' + str(thread_id) + ', self.ReqIDIdle[thread_id] = ' + str(self.ReqIDIdle[thread_id]))
