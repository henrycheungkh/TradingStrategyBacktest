...
class IBapiUSStocksGapperScanner(IBapiDataReader):
    ...
    def historicalData(self, reqId:int, bar: BarData):
        ...
        d = vars(bar)
        if self.PreRun:
            print('historicalData, thread_id = ' + str(thread_id))
            print(d)
        else:
            print('historicalData, thread_id = ' + str(thread_id))
            print(d)
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        thread_id = reqId - self.RequestID_Range[0]
        if self.PreRun:
            print('historicalDataEnd, thread_id = ' + str(thread_id))
        else:
            print('historicalDataEnd, thread_id = ' + str(thread_id))
        self.PreRun = False
    ...
    def TriggerScanning(self, thread_id, PriceType = 'TRADES'):
        if thread_id < self.AllReqIDCount:
            ...
            TickerIndex = self.TickerIndexOffset[thread_id] * self.AllReqIDCount + thread_id
            print('TriggerScanning for thread_id = ' + str(thread_id) + ', TickerIndex = ' + str(TickerIndex) + ', symbol = ' + str(self.TickerList.loc[TickerIndex, 'symbol']))
            try:
                contract = self.getIBContract(self.TickerList.loc[TickerIndex, 'symbol'], self.TickerList.loc[TickerIndex, 'secType'], self.TickerList.loc[TickerIndex, 'exchange'], self.TickerList.loc[TickerIndex, 'currency'], self.TickerList.loc[TickerIndex, 'primaryExchange'])
                self.IBProcessHub.reqHistoricalData(thread_id + IBapiUSStocksGapperScanner.RequestIDRange[0], contract, self.DataEndTime, "1 D", "1 day", PriceType, 0, 1, False, [])
            except:
                print('Error in TriggerScanning: TickerIndex is ' + str(TickerIndex))
    ...
    def RunProcess(self, IBProcessHub):
        ...
        while (True):
            print('while loop, thread_id = ' + str(thread_id))
            time.sleep(10)
