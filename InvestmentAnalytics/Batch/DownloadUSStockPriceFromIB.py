...
if len(sys.argv) > 1:
    ...
    StartingTickerBatchID = int(sys.argv[3])
else:
...
if len(sys.argv) > 4:
    Data_Items = sys.argv[4].split(",")
else:
    # Data_Items = ['TRADES','BID','ASK']
    # Data_Items = ['BID']    
    # Data_Items = ['ASK']    
    Data_Items = ['ADJUSTED_LAST']    

if len(sys.argv) > 5:
    HistoricalPeriod = sys.argv[5]
else:
    # HistoricalPeriod = "3 D"
    HistoricalPeriod = "2 Y"

if len(sys.argv) > 6:
    DirectUpload = sys.argv[6]
else:
    DirectUpload = "No Upload"
    # DirectUpload = "DirectUpload"
    
if len(sys.argv) > 7:
    DataEndDate = sys.argv[7] 
    print(DataEndDate)
...
# Tickers = Tickers[Tickers['symbol'].isin(["AAPL", "NVDA"])].copy()

# Tickers.to_csv(r'd:\temp\FullTickers.csv', index=False)
...
# DataEndDateString = datetime.strptime(DataEndDate + " 23:59:59", "%Y%m%d %H:%M:%S")
    ...
    if (DirectUpload == "DirectUpload"):
        ...
        DBExportDirectUpload(DatafilePath + 'UploadScript.sql', TableNameMapping[BarSize], DatabaseName = DBNameMapping[BarSize], UploadToDB = False, Priority = 10)
