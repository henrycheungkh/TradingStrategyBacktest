...
def DBDirectUpload(command_without_quote, DatabaseName, DBTableName, CountRecordCount = True):
    print('Start to execute DB Upload command: ' + command_without_quote + ' at ' + str(datetime.now()))
    ...
    statement = text("""UPDATE pending_db_upload_command SET Uploaded = True WHERE command = :command AND DBName = :DBName AND TableName = :TableName""")
    line = { "command": command_without_quote, "DBName": DatabaseName, "TableName": DBTableName }
    engine = GetSQLAlchemyEngine()
    # engine.execute(statement, **line)
    with engine.connect() as conn:
        # result = conn.execute(statement, line)
