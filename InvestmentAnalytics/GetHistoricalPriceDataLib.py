...
from InvestmentAnalytics.MarketDataReader import getAnalysisContext
...
def getListOfTradingMoments(StartDate, EndDate, TimeFrame, TickerList, SharpeRatioThreshold, TradeMomentPeriod, ObservationPeriod, AfterMomentPeriod, InstrumentType = 'Futures'):

    analysis_context = getAnalysisContext(StartDate, EndDate, TimeFrame, TickerFilter = TickerList, KeepDataframeData = True)
    # full_data = analysis_context.FuturesData.ActiveContractPrices[['ticker', 'tDateTime', 'high', 'low', 'open', 'close', 'high_adj', 'low_adj', 'open_adj', 'close_adj', 'vol']]
    full_data = analysis_context.FuturesData.ActiveContractPrices[['ticker', 'tDateTime', 'high', 'low', 'open', 'close', 'high_adj', 'low_adj', 'open_adj', 'close_adj', 'vol', 'ticker id', 'TimeInStandardUnit', 'MarketTimeSectionID']]
    # full_data = analysis_context.FuturesData.ActiveContractPrices
    
    # full_data = full_data.rename(columns={"tDateTime": "dates", "vol": "volume"})

    return analysis_context, full_data


def getHistoricalPriceGraphData(StartDateString, EndDateString, TimeFrame, TickerList, InstrumentType = 'Futures'):

    if len(TickerList) == 1:
        tickerfilter = "ticker = '" + TickerList[0] + "'"
    else:
        tickerfilter = "ticker in ('RTY')"
    ...
    if TimeFrame == "1 min":
        ...
        DB_and_table_pair_list = [
          # {"database": "finance_fdata_fut_hist", "table": "`fdata_fut_hist`"},
          {"database": "finance_fdata_fut_hist_10secs", "table": "`fdata_fut_hist`"},
...
# data = getHistoricalPriceGraphData(StartDateString, EndDateString, TimeFrame, ['RTY'])
