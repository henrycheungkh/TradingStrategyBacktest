...
trade_result = np.zeros((InitialResultCacheSize, RESULT_COLUMN_COUNT)).astype(np.float32) 
...
mod = SourceModule("""
  ...
  __global__ void correlation_on_specific_time_section_analysis(int InitialResultCacheSize, int RESULT_COLUMN_COUNT, int *trade_result_count, float *trade_result)
  {
...
//    for (int i=0; i<2000000; i++) {
      ...
      if (trade_result_index < InitialResultCacheSize) {
        trade_result[trade_result_index * RESULT_COLUMN_COUNT] = InitialResultCacheSize;
      }
    }
  }
  """)
...
func( np.int32(InitialResultCacheSize), np.int32(RESULT_COLUMN_COUNT), trade_result_count_gpu, trade_result_gpu, block=(4,16,16))

trade_record = np.empty((InitialResultCacheSize, RESULT_COLUMN_COUNT), dtype=np.float32)
