...
mod = SourceModule("""
  __device__ float add_one( float in); 

  __global__ void matrix_location_trial(float *in_matrix, float *out_matrix)
  {
     out_matrix[0] = add_one(in_matrix[0]);
     out_matrix[1] = add_one(in_matrix[1]);
     out_matrix[2] = add_one(in_matrix[2]);
     out_matrix[3] = add_one(in_matrix[3]);
     out_matrix[4] = add_one(in_matrix[6]);
     out_matrix[5] = add_one(in_matrix[9]);
  }
  
  __device__ float add_one( float input_float)
  {
    return input_float + 1;
  }
  """)
