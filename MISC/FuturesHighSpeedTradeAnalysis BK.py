...
FuturesData =  {'TRADES':pd.DataFrame(),'BID':pd.DataFrame(),'ASK':pd.DataFrame()}
...
DownloadError = False
app.clearCache()
queryTime = "20210510 24:00:00"
app.reqHistoricalData(Request_ID, contract, queryTime, HistoricalPeriod, BarSize, Request_Data_Item, 0, 1, False, [])
...
if not DownloadError:
    ...
    FuturesData[Request_Data_Item] = app.df[Request_Data_Item].copy().reset_index()
    
    FuturesData[Request_Data_Item]['date'] = pd.to_datetime(FuturesData[Request_Data_Item]['date']).dt.tz_localize('Europe/London')
    FuturesData[Request_Data_Item]['date (US timezone)'] = FuturesData[Request_Data_Item].apply(lambda x: x['date'].tz_convert('US/Central'), axis = 1)
    FuturesData[Request_Data_Item]['date (US timezone) hour'] = FuturesData[Request_Data_Item]['date (US timezone)'].dt.hour
    FuturesData[Request_Data_Item]['date (US timezone) minute'] = FuturesData[Request_Data_Item]['date (US timezone)'].dt.minute
    
    # FuturesData[Request_Data_Item]['date (US timezone)'] = FuturesData[Request_Data_Item]['date']\.tz_convert('US/Central')
    
    print('FuturesData at a')
    print(FuturesData[Request_Data_Item])
    csvfilepath = Config.CONFIG_BASE_DatafilePath + '\Futures Dump\Futures Dump ' + Request_Data_Item + ' ended 20210510 ' + BarSize + '.csv'
