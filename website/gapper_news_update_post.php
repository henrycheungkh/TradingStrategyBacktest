...
if (isset($_POST['CaptureDate'])) {
  ...
  while(isset($_POST['Ticker'.$x])) {
    ...
    mysqli_query($conn, "INSERT INTO fdata_us_gapper_list_news VALUES ('" . $_POST['CaptureDate'] . "', '" . $_POST['ticker'.$x] . "', '" . $_POST['title'.$x] . "', '" . $_POST['media'.$x] . "', '" . $_POST['date_desc'.$x] . "', '" . $_POST['datetime'.$x] . "', '" . $_POST['news_desc'.$x] . "', '" . $_POST['link_desc'.$x] . "' ) ON DUPLICATE KEY UPDATE media = '" . $_POST['media'.$x] . "'");
    $x++;
  }
  mysqli_commit($conn);
}
