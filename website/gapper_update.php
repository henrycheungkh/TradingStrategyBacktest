...
if (isset($_GET['Delete'])) {
  ...
  if ($_GET['Delete'] == 'Yes') {
      mysqli_query($conn, "DELETE FROM fdata_us_gapper_list where CaptureDate = '" . $_GET['CaptureDate'] . "'");
  }
  
  while(isset($_GET['Ticker'.$x])) {
    echo $_GET['Ticker'.$x] . '<BR>';
    mysqli_query($conn, "INSERT INTO fdata_us_gapper_list VALUES ('". $_GET['CaptureDate'] . "', '" . $_GET['Ticker'.$x] . "', '" . $_GET['Sector'.$x] . "', '" . $_GET['Industry'.$x] . "', " . $_GET['PriorDayClose'.$x] . ", " . $_GET['CurrentPrice'.$x] . ", " . $_GET['MA30Vol'.$x] . ", " . $_GET['TodayVol'.$x] . ", " . $_GET['MarketCap'.$x] . ", " . $_GET['FreeFloat'.$x]. ", " . $_GET['BidAskSpread'.$x] . ", NULL, NULL)");
    $x++;
  }
  mysqli_commit($conn);
}
