...
if (isset($_POST['CaptureDate'])) {
  ...
  mysqli_query($conn, "DELETE FROM fdata_us_gapper_list where CaptureDate = '" . $_POST['CaptureDate'] . "'");
  
  while(isset($_POST['Ticker'.$x])) {
    echo $_POST['Ticker'.$x] . '<BR>';
    mysqli_query($conn, "INSERT INTO fdata_us_gapper_list VALUES ('". $_POST['CaptureDate'] . "', '" . $_POST['Ticker'.$x] . "', '" . $_POST['Sector'.$x] . "', '" . $_POST['Industry'.$x] . "', " . $_POST['PriorDayClose'.$x] . ", " . $_POST['CurrentPrice'.$x] . ", " . $_POST['MA30Vol'.$x] . ", " . $_POST['TodayVol'.$x] . ", " . $_POST['MarketCap'.$x] . ", " . $_POST['FreeFloat'.$x]. ", " . $_POST['BidAskSpread'.$x] . ", NULL, NULL)");
    $x++;
  }
  mysqli_commit($conn);
}
