...
$sql = "SELECT *, (CurrentPrice - PriorDayClose)/PriorDayClose AS GapSize, Today_Vol/(30MA_Vol+0.001) AS RelativeVol FROM fdata_us_gapper_list WHERE Today_Vol/30MA_Vol > 3 AND MarketCap > 100000000 AND ABS((CurrentPrice - PriorDayClose)/PriorDayClose) > 0.04 AND CaptureDate = '" . $date->format('Y-m-d') . "' ORDER BY ABS((CurrentPrice - PriorDayClose)/PriorDayClose) DESC";
